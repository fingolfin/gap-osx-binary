...
CTblLib.Test.FusionToTom:= function( tbl )
    local tom, result, fustotom, tommaxes, tblmaxes, orders, primperm, t,
          nam, pos, fusrec, storedfus, fus, compat1, compat2, map,
          tomprimperm, tblfustom, pair, supertom, tomfus, supertblname,
          supertbl, tblfus, supertblfussupertom, r, altern, composition;
    ...
    if IsBound( fusrec.text ) then
      ...
      if Length( fus ) = 1
        ...
        result:= false;
      elif 1 < Length( fus ) and
         ( fusrec.text = "fusion map is unique"
    ...
    if 1 < Length( RepresentativesFusions( AutomorphismsOfTable( tbl ), fus,
      ...
      # Check that the text does not lie.
      if IsBound( fusrec.text ) and 1 < Length( fus ) and
         PositionSublist( fusrec.text, "unique up to table " ) = fail then
        CTblLib.PrintTestLog( "E", "CTblLib.Test.FusionToTom",
            Identifier( tbl ),
      ...
      if storedfus = fail or not storedfus in compat1 then
        ...
        if not IsBound( fusrec.perm ) then
          ...
          if IsEmpty( compat2 ) then
            ...
            CTblLib.PrintTestLog( "E", "CTblLib.Test.FusionToTom",
                Identifier( tbl ),
                "stored fusion to tom needs the following perm. of maxes" );
            Print( LibraryFusionTblToTom( tbl, tblfustom ) );
          else
            CTblLib.PrintTestLog( "E", "CTblLib.Test.FusionToTom",
                Identifier( tbl ),
                "stored fusion to tom needs a permutation of maxes" );
          fi;
        fi;
...
Add( CTblLib.HardTableAutomorphisms, "2.O8+(7)" );
