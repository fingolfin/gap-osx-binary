...
# Check that this has not been already include by HAP
# (it shouldn't be, but earlier versions of HAP included a copy)
if not IsBoundGlobal("MakeHAPprimeDoc") then
#####################################################################
...
InstallGlobalFunction(MakeHAPprimeDoc,
  function(arg)
    ...
    userguideGAP4stones := 6870000000;
    datatypesGAP4stones := 2880000000;
...
##  - check test ReadTest("/home/pas/GAP/gap/pkg/HAPprime/tst/happrime.tst");
##  - tweak GAP4stones number in tst/happrime.tst to be between 300000-400000 
##      (i.e. the same as given by ReadTest("/home/pas/local/lib/gap4r4/tst/combinat.tst"); )
