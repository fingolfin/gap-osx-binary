...
############################################################################# 
## 
#M  MultiplicativeElementWithObjects( <mwo>, <elt>, <tail>, <head> ) 
#M  MultiplicativeElementWithObjectsNC( <isgpdelt>, <elt>, <tail>, <head> ) 
##
InstallMethod( MultiplicativeElementWithObjectsNC, 
...
InstallMethod( MultiplicativeElementWithObjects, 
    ...
    return MultiplicativeElementWithObjectsNC( false, e, t, h ); 
end );
...
#?            which is why there is a special IsElementOfGroupoid in gpd.gi
...
InstallMethod( IsElementOfMagmaWithObjects, 
...
function( e, mwo )
    return IsElementOfMagmaWithObjects( e, PieceOfObject( mwo, e![2] ) ); 
end );
...
function( mwo )
    ...
    if ( type > 2 ) then 
        ...
        if ( type = 4 ) then ## a groupoid 
            ...
            gens1 := List( mgens, 
                g -> MultiplicativeElementWithObjectsNC( false, g, o1, o1 ) );
            ...
            for i in obs do 
                for j in obs do 
                    if ( i <> j ) then 
                       k := k+1; 
                       gens2[k] := 
                          MultiplicativeElementWithObjectsNC( false, id, i, j ); 
                    fi;
                od;
            od;
            gens := Immutable( Concatenation( gens1, gens2 ) ); 
    ...
    ## add in all possible generators? 
        ...
        for i in obs do 
           for j in obs do 
              for g in mgens do 
                k := k+1; 
                gens[k] := MultiplicativeElementWithObjectsNC( false, g, i, j );
              od;
           od; 
        od;
...
function( dwo )
    ...
    if not IsDiscreteDomainWithObjects( dwo ) then 
        Error( "no method yet installed in the case when dwo not discrete," ); 
    fi;
    pieces := Pieces( dwo ); 
    g := pieces[1]!.magma; 
...
function( mwo )
    return IteratorByFunctions( rec( 
        ...
        NextIterator := function( iter )
            ...
            if ( HasIsDirectProductWithCompleteGraph( mwo ) 
                 and IsDirectProductWithCompleteGraph( mwo ) ) then 
               return MultiplicativeElementWithObjects( mwo, iter!.melt, 
