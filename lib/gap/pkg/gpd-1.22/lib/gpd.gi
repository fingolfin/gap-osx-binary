...
#W                                                               & Emma Moore
##  version 1.22, 05/11/2013 
##
#Y  Copyright (C) 2013, Emma Moore and Chris Wensley,  
...
function( gpd ) 
    ...
    rays := gpd!.rays; 
    return List( [1..nobs], 
               i -> GroupoidElement( gpd, rays[i], obs[1], obs[i] ) ); 
end );
...
function( gpd )
    ...
    if IsSinglePiece( gpd ) then 
        if IsDirectProductWithCompleteGraph( gpd ) then 
            ...
            Print( "larger direct product groupoid: ", pgpd, "\n" ); 
            Print( "     objects: ", gpd!.objects, "\n" );
            rgp := gpd!.magma;
            Print( "  root group: " );
            if HasName( rgp ) then
...
function( gpd, oblist )
    ...
    if not ForAll( oblist, L -> ( Length(L) = len ) ) then 
        Error( "lists in oblist must have the same length as Objects(gpd)," ); 
    fi; 
    ob1 := oblist[1]; 
...
#M  GroupoidElement 
##
InstallMethod( GroupoidElement, "generic method for a groupoid element",
...
function( gpd, g, i, j )
    ...
    if not ( ok1 and ok2 ) then 
        ...
        e :=  MultiplicativeElementWithObjectsNC( true, g, i, j ); 
...
function( gpd, elt )
    ...
    ims := List( gens, g -> ConjugateGroupoidElement( g, elt ) ); 
        ...
        NextIterator := function( iter )
            ...
            if ( hc![6] in [ "c", "r" ] ) then 
                return    ## GroupoidElement( iter!.fgpd!.mwo, 
                    MultiplicativeElementWithObjectsNC( 
                        true, (hc![5][iter!.tpos])*(iter!.gpelt), 
                        iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            elif ( hc![6] in [ "s", "h" ] ) then 
                return    ## GroupoidElement( iter!.fgpd!.mwo, 
                    MultiplicativeElementWithObjectsNC( 
                        true, (iter!.gpelt)*(hc![5][iter!.hpos]), 
                        iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            elif ( hc![6] in [ "l" ] ) then 
                return    ## GroupoidElement( iter!.fgpd!.mwo, 
                    MultiplicativeElementWithObjectsNC( 
                        true, (iter!.gpelt)^(-1)*(hc![5][iter!.hpos]), 
                        iter!.tobs[iter!.tpos], iter!.hobs[iter!.hpos] );
            elif ( hc![6] in [ "d" ] ) then 
...
function( gpd )
    return IteratorByFunctions( rec( 
        ...
        NextIterator := function( iter )
            ...
            if ( HasIsDirectProductWithCompleteGraph( gpd ) 
                 and IsDirectProductWithCompleteGraph( gpd ) ) then 
               #? (08/09/10) removed the NC and added back the .mwo
               #? return GroupoidElement( 
               #?    FamilyObj(gpd)!.mwo, 
               #? (21/09/10) changed again, using the new MuElWiObNC 
                return MultiplicativeElementWithObjectsNC( true, 
                   iter!.gpelt, iter!.obs[iter!.tpos], iter!.obs[iter!.hpos] );
            else 
               #? return GroupoidElement( 
               #?    FamilyObj( Parent(gpd) )!.mwo, 
                return MultiplicativeElementWithObjectsNC( true, 
...
##
InstallMethod( ReplaceOnePieceInUnion, "for union groupoid and two groupoids", 
    true, [ IsGroupoid and IsPiecesRep, IsGroupoid and IsSinglePiece, 
    IsGroupoid and IsSinglePiece ], 0,
function( U, old, new )

    local  ro, pn, pieces; 
    ro := RootObject( old ); 
    pn := PieceNrOfObject( U, ro ); 
    if ( pn = fail ) then 
        return fail; 
    fi; 
    pieces := ShallowCopy( Pieces( U ) ); 
    if not ( pieces[pn] = old ) then 
        Info( InfoGpd, 1, "old not a piece of U" ); 
        return fail; 
    fi; 
    ...
    for cu in Pieces( U ) do
        ...
        for cv in Pieces( V ) do
            ...
            for c in dc do
                r := Representative( c );
                #? (08/09/10) removed NC and added back the G 
                Add( reps, GroupoidElement( G, r, ou, ov ) );
            od;
        od;
    od;
    return reps;
...
function( e1, e2 )
    ...
    return MultiplicativeElementWithObjectsNC( 
        true, e1![1]*e2![1], e1![2], e2![3] );
end );
...
function( e, n )
    ...
    if ( n = 1 ) then
        ...
        return MultiplicativeElementWithObjectsNC( true, e![1]^-1, h, t );
    elif ( t = h ) then
        return MultiplicativeElementWithObjectsNC( true, e![1]^n, t, h );
    fi; 
    return fail; 
end );
...
InstallMethod( ConjugateGroupoidElement, "for two groupoid elements", 
    ...
    if ( e2![3] = p ) then  
        ...
        if ( e1![2] = p ) then 
            if ( e1![3] = p ) then
                ...
                return MultiplicativeElementWithObjectsNC( 
                    true, c^(-1)*e1![1]*c^(-1), q, p ); 
            else 
                return MultiplicativeElementWithObjectsNC( 
                    true, c^(-1)*e1![1], q, e1![3] ); 
            fi; 
        elif ( e1![2] = q ) then 
            if ( e1![3] = q ) then 
                return MultiplicativeElementWithObjectsNC( 
...
#M  IdentityElement
## 
InstallMethod( IdentityElement, "for a connected groupoid and object",
    true, [ IsGroupoid and IsSinglePiece, IsScalar ], 0,
function( gpd, obj )
    return MultiplicativeElementWithObjectsNC( 
        true, One(gpd!.magma), obj, obj );
end );

InstallMethod( IdentityElement, "generic method for groupoid and object",
    true, [ IsGroupoid, IsScalar ], 0,
function( gpd, obj )
    local  comp;
    comp := PieceOfObject( gpd, obj );
    return MultiplicativeElementWithObjectsNC( 
        true, One(comp!.magma), obj, obj );
end );
