    ...
    if IsSinglePiece( sgpd ) then
        ...
        if IsSinglePiece( gpd ) then
            ...
            return GroupoidHomomorphismFromSinglePieceNC( 
                       sgpd, gpd, inc, sobs, RaysOfGroupoid( sgpd ) );
        else
            o1 := sgpd!.objects[1];
            ...
            mor := GroupoidHomomorphismFromSinglePieceNC( 
                       sgpd, c1, inc, sobs, RaysOfGroupoid( sgpd ) );
...
function( mor, U, V )
    ...
    for j in [1..lenU] do
        ...
        mors[j] := GroupoidHomomorphismFromSinglePieceNC( 
                       sj, rng, orj, rhom, RaysOfGroupoid( rng ) );
    od;
...
function( mor, obj ) 
    ...
    if ( HasIsGeneralMappingFromSinglePiece( mor ) 
        ...
        if ( imobs[1] = obr[1] ) then  ## root maps to root 
            ...
            mgi := MappingGeneratorsImages( roh ); 
            ray := RaysOfGroupoid( rng )[ Position( obr, imobs[1] ) ]; 
...
function( src, rng, hom )
    return GroupoidHomomorphismFromSinglePieceNC( 
               src, rng, hom, rng!.objects, RaysOfGroupoid( rng ) ); 
end ); 
...
function( m1, m2 )
    ...
    Info( InfoGpd, 2, "method 2 for * with maps from hom discrete gpds" );
    s1 := Source( m1 ); 
    r1 := Range( m1 );
    s2 := Source( m2 );
    r2 := Range( m2 );
...
function ( map, e )
    ...
    return MultiplicativeElementWithObjectsNC( true, g2, imo[pt1], imo[ph1] );
end ); 
...
function ( map, e )
    ...
    return MultiplicativeElementWithObjectsNC( true, g2, t2, t2 );
end ); 
...
function( g1 )
    ...
    iso := GroupoidHomomorphismFromSinglePieceNC( 
               g1, g2, obs, g2p, RaysOfGroupoid( g2 ) );
    SetIsInjectiveOnObjects( iso, true );
    SetIsSurjectiveOnObjects( iso, true );
    return iso;
end );
