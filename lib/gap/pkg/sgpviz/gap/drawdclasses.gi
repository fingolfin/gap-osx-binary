...
# dclasses[i] <= dclasses[j]
          file,                       # name of the file where dot code
# will be written
...
# the user.
    ...
    idempots2 := List(idempots, x -> ImageListOfTransformation(x));
...
# and determine the "zero" digit
    is_partial := ForAll(List(generators, g -> ImageListOfTransformation(g)), lx -> lx[tlen] = tlen);
    zero := tlen;
...
# in the current D-class node
                AppendTo(file, "<TD BORDER=\"0\">");  # opens h-class
...
# write the elements of current H-class
				AppendTo(file, "<TABLE CELLSPACING=\"0\">");
				for k in [1..Length(bag2[i][j])] do
					...
					while k2 > 0 do
						...
						k2 := k2 - 1;
					od;
                	AppendTo(file, "<TR><TD BGCOLOR=\"", color, "\" BORDER=\"0\">");
                    AppendTo(file, bag2[i][j][k]);
                    AppendTo(file, "</TD></TR>\n");
                od;
                AppendTo(file, "</TABLE>");

                AppendTo(file, "</TD>");	# close H-class
